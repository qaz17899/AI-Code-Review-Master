...
const TreeNode: React.FC<TreeNodeProps> = ({
    ...
    if (node.type === 'folder') {
        ...
        return (
            <div className="animate-fade-in-up" style={{ animationDuration: '300ms', animationDelay: `${animationIndex * 30}ms`, opacity: 0 }}>
                <div className="flex items-center gap-2 group text-sm p-0.5 rounded-md hover:bg-stone-300/60 dark:hover:bg-slate-700/50">
                    <button onClick={() => onToggleFolder(node.path)} className="flex items-center gap-2 flex-grow text-left" style={{ paddingLeft: `${level * 1.25}rem` }}>
                        {isExpanded ? <ChevronDownIcon className="h-4 w-4 flex-shrink-0" /> : <ChevronRightIcon className="h-4 w-4 flex-shrink-0" />}
                        <div onClick={(e) => {e.stopPropagation(); onToggleFileSelection(node.path, true)}} className={`w-5 h-5 flex-shrink-0 border-2 rounded-md flex items-center justify-center transition-all duration-150 active:scale-90 ${selectionState !== 'none' ? 'accent-gradient-bg border-transparent' : 'border-stone-500 dark:border-slate-600 group-hover:border-[var(--accent-color)]'}`}>
    ...
    return (
        <div className="flex items-center gap-2 group text-sm p-0.5 rounded-md hover:bg-stone-300/60 dark:hover:bg-slate-700/50 animate-fade-in-up" style={{ animationDuration: '300ms', animationDelay: `${animationIndex * 30}ms`, opacity: 0 }}>
            <button onClick={() => onToggleFileSelection(node.path, false)} className="flex items-center gap-2 flex-grow text-left" style={{ paddingLeft: `${level * 1.25 + 0.5}rem` }}>
                ...
                <div className={`w-5 h-5 flex-shrink-0 border-2 rounded-md flex items-center justify-center transition-all duration-150 active:scale-90 ${isSelected ? 'accent-gradient-bg border-transparent' : 'border-stone-500 dark:border-slate-600'}`}>
