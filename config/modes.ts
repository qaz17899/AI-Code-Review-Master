...
export const MODES: Record<ReviewMode, ModeConfig> = {
    ...
    REVIEW: {
        ...
        ui: {
            subTitle: 'Your Personal Senior Code Review Expert',
    ...
    REFACTOR: {
        ...
        example: { title: '重構 (Refactor)', prompt: `function printOrder(order) {\n  let total = order.price * order.quantity;\n  if (order.isDiscounted) {\n    total *= 0.9;\n  }\n  console.log("Total price:", total);\n}`, response: `### 重構建議\n目前的函式混合了計算和輸出的職責，可以將其拆分為兩個獨立的函式，以符合單一職責原則。\n\n### 修改後\n\`\`\`javascript\nfunction calculateTotal(order) {\n  let total = order.price * order.quantity;\n  if (order.isDiscounted) {\n    return total * 0.9;\n  }\n  return total;\n}\n\nfunction printOrder(order) {\n  const total = calculateTotal(order);\n  console.log("Total price:", total);\n}\n\`\`\`` },
        ui: {
            subTitle: 'Your Automated Code Refactoring Partner',
    ...
    DOCS: {
        ...
        example: { title: '產生文件 (Docs)', prompt: `function calculateArea(radius) {\n  return Math.PI * radius * radius;\n}`, response: `### JSDoc 註解\n\n\`\`\`javascript\n/**\n * 計算圓形的面積。\n * @param {number} radius - 圓形的半徑。\n * @returns {number} 圓形的面積。\n */\nfunction calculateArea(radius) {\n  return Math.PI * radius * radius;\n}\n\`\`\`` },
        ui: {
            subTitle: 'Your Automated Documentation Specialist',
    ...
    TESTER: {
        ...
        example: { title: '產生測試 (Tester)', prompt: `// utils.js\nexport function slugify(text) {\n  return text.toLowerCase().replace(/\\s+/g, '-').replace(/[^\\w-]+/g, '');\n}`, response: `### 測試程式碼 (Jest)\n\n\`\`\`javascript\n// utils.test.js\nimport { slugify } from './utils';\n\ndescribe('slugify', () => {\n  test('應將空格轉換為連字號', () => {\n    expect(slugify('hello world')).toBe('hello-world');\n  });\n\n  test('應移除特殊字元', () => {\n    expect(slugify('hello!@#world?')).toBe('helloworld');\n  });\n\n  test('應轉換為小寫', () => {\n    expect(slugify('Hello World')).toBe('hello-world');\n  });\n});\n\`\`\`` },
        ui: {
            subTitle: 'Your Dedicated Test Generation Engineer',
    ...
    DESIGN: {
        ...
        example: { title: '系統設計 (Design)', prompt: `我需要一個簡單的待辦事項 (To-Do) 應用的 API 設計。`, response: `### To-Do API 設計\n\n#### 資料模型\n\`\`\`json\n{\n  "id": "string",\n  "text": "string",\n  "completed": "boolean",\n  "createdAt": "timestamp"\n}\n\`\`\`\n\n#### API 端點\n- \`GET /todos\`: 獲取所有待辦事項\n- \`POST /todos\`: 新增一筆待辦事項\n- \`PUT /todos/:id\`: 更新一筆待辦事項\n- \`DELETE /todos/:id\`: 刪除一筆待辦事項` },
        ui: {
            subTitle: 'Your Expert System Architect',
    ...
    IMPLEMENT: {
        ...
        example: { title: '實作程式碼 (Implement)', prompt: `設計方案：\n- 函式名稱: \`greet\`\n- 參數: \`name\` (string)\n- 回傳: 一個 "Hello, [name]!" 的字串`, response: `### 程式碼實作\n\n\`\`\`javascript\n/**\n * 產生問候語字串。\n * @param {string} name - 要問候的人名。\n * @returns {string} 問候語。\n */\nfunction greet(name) {\n  return \`Hello, \${name}!\`;\n}\n\`\`\`` },
        ui: {
            subTitle: 'Your Efficient Code Implementation Partner',
    ...
    VERIFY: {
        ...
        example: { title: '驗證實作 (Verify)', prompt: `**修改方案:**\n1. 函式 \`getUser\` 應改為非同步函式 (async)。\n2. 若 \`id\` 不存在，應拋出錯誤。\n\n**程式碼:**\n\`\`\`javascript\nfunction getUser(id) {\n  if (id) {\n    return fetch(\`/api/users/\${id}\`);\n  }\n}\n\`\`\``, response: `### 驗證結果\n\n- **修改方案 1 (改為 async):**\n    - **狀態:** 🔴 未完成\n    - **說明:** 函式簽名未加上 \`async\` 關鍵字。\n- **修改方案 2 (拋出錯誤):**\n    - **狀態:** 🔴 未完成\n    - **說明:** 當 \`id\` 不存在時，函式僅返回 \`undefined\`，並未拋出錯誤。\n\n**結論:** 程式碼未完全符合修改方案。` },
        ui: {
            subTitle: 'Your Implementation Verification Specialist',
    ...
    DESIGNER: {
        ...
        example: { title: '遊戲設計 (Designer)', prompt: `我想設計一個新的法師技能：「冰霜新星」。它應該是一個範圍傷害技能，並帶有控場效果。`, response: `### 技能設計：冰霜新星\n\n| 屬性 | 設計內容 |\n| :--- | :--- |\n| **技能名稱** | 冰霜新星 (Frost Nova) |\n| **目標** | 施法者周圍半徑 5 公尺內的所有敵人 |\n| **傷害** | 50 點冰霜傷害 |\n| **控場效果** | **冰凍**: 使目標無法移動，持續 2 秒 |\n| **冷卻時間** | 10 秒 |\n| **消耗** | 30 點法力 |\n\n**設計理念:** 這是一個中等傷害的控場技能，適合用於拉開距離或為後續高傷害技能做準備。` },
        ui: {
            subTitle: 'Your Creative Game Design Partner',
    ...
    ENHANCE: {
        ...
        example: { title: '功能強化 (Enhance)', prompt: `這是一個簡單的計數器應用，只有一個按鈕可以增加數字。`, response: `### 功能強化建議\n\n#### 1. 核心體驗優化\n- **新增「減少」按鈕:** 提供對稱的操作，讓使用者可以修正錯誤。\n- **新增「重設」按鈕:** 讓使用者可以快速將計數歸零。\n\n#### 2. 功能擴展\n- **步進值設定:** 允許使用者設定每次點擊增加或減少的數值（例如，一次 +5）。\n- **儲存功能:** 使用者關閉瀏覽器後，計數器的值能夠被保存並在下次開啟時恢復。` },
        ui: {
            subTitle: 'Your Product Experience Enhancement Expert',
